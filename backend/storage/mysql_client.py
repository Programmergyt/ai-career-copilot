...
def get_mysql_pool() -> PooledDB:
    ...
    if _pool is None:
        ...
        logger.info("MySQL pool initialized: %s:%s/%s", cfg["host"], cfg["port"], cfg["database"])
    return _pool
...
class MySQLStore:
    ...
    def __init__(self) -> None:
        self._pool = get_mysql_pool()

    def _conn(self):
        return self._pool.connection()

    # ---- sessions ----
    def upsert_session(self, session_id: str, status: str = "active") -> None:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(sql, (session_id, now, now, status, now, status))
            conn.commit()
        finally:
            conn.close()
    ...
    def _get_json(self, table: str, row_id: str) -> dict | None:
    ...
    def _get_by_session(self, table: str, session_id: str) -> dict | None:
    ...
    def save_resume_content(self, row_id: str, session_id: str, data: dict, version: int = 1,
    ...
    def save_resume_html(self, row_id: str, session_id: str, html: str, version: int = 1,
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(sql, (row_id, session_id, version, html, content_ver, render_ver, checksum, now,
